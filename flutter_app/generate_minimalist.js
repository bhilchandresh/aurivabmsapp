...
class MinimalistTemplate extends StatelessWidget {
  ...
  Widget _buildSignatorySection() {
    final signatureImage = params.tenant['signatureImage'];
    final name = params.tenant['name'] ?? '';
    ...
    return Align(
      ...
      child: SizedBox(
        ...
        child: Column(
          children: [
            if (signatureImage != null && signatureImage.toString().isNotEmpty)
              _buildLogoWidget(signatureImage, height: 40, width: 140, fit: BoxFit.contain)
            else
              Container(
                height: 40,
                width: 140,
...
fs.writeFileSync('lib/features/invoices/templates/minimalist_template.dart', dartCode);
fs.writeFileSync('lib/features/quotations/templates/minimalist_template.dart', dartCode);
